...
function computeStats(
	...
	const trainingDays = countDays(
		yearHols.filter((h) => h.type === "training"),
	);
...
export function HolidaysPage() {
	...
	function handleSubmitRequest() {
		...
		createHoliday({
			...
			date: reqDate,
			endDate:
				reqEndDate && reqEndDate > reqDate ? reqEndDate : undefined,
			halfDay: reqHalfDay,
					...
					<section className="mb-8">
						...
						<div className="space-y-2">
							{pendingRequests.map((h) => {
								...
								const typeCfg = HOLIDAY_TYPE_OPTIONS.find(
									(t) => t.value === h.type,
								);
								return (
									<div
							...
							<tbody>
								{engineers.map((eng) => {
									const stats = computeStats(
										...
										eng.id,
										selectedYear,
										eng.holidayAllowance,
									);
									const isEditingAllowance =
										editAllowance === eng.id;
									return (
										<tr
											key={eng.id}
										...
										>
											...
											<td className="text-center px-3 py-2.5 text-blue-300">
												{stats.holidayDays}
											</td>
											<td
												className={`text-center px-3 py-2.5 font-medium ${stats.remaining < 0 ? "text-red-400" : stats.remaining <= 5 ? "text-amber-500" : "text-green-400"}`}
											>
												{stats.remaining}
											</td>
											<td className="text-center px-3 py-2.5 text-red-300">
												{stats.sickDays}
											</td>
											<td className="text-center px-3 py-2.5 text-green-300">
												{stats.trainingDays}
											</td>
											<td className="text-center px-3 py-2.5 text-neutral-400">
												{stats.otherDays}
											</td>
										</tr>
									);
								...
								{yearHols.map((h) => {
									...
									return (
										...
										>
											...
											<span
												className={`text-[10px] px-2 py-0.5 rounded-full ${statusBadge.bg} ${statusBadge.text}`}
											>
												{statusBadge.label}
											</span>
										</div>
	...
	const myStats = currentUser
		? computeStats(
				...
				currentUser.id,
				selectedYear,
				currentUser.holidayAllowance,
			)
		: null;
	const myHolidays = holidays
		.filter(
			(h) =>
				h.profileId === currentUser?.id &&
				h.date.startsWith(String(selectedYear)),
		)
		.sort((a, b) => a.date.localeCompare(b.date));
		...
		<div className="p-6 md:p-8 max-w-3xl">
			...
			{myStats && (
				<div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
					...
					<div
						className={`rounded-xl border p-3 text-center ${myStats.remaining < 0 ? "border-red-800/50 bg-red-950/20" : myStats.remaining <= 5 ? "border-amber-800/50 bg-amber-950/20" : "border-neutral-800 bg-neutral-900"}`}
					>
						<p
							className={`text-2xl font-bold ${myStats.remaining < 0 ? "text-red-400" : myStats.remaining <= 5 ? "text-amber-500" : "text-green-400"}`}
						>
							{myStats.remaining}
						</p>
						<p className="text-[11px] text-neutral-500">
							Remaining (of {myStats.allowance})
						</p>
					</div>
					<div className="rounded-xl border border-neutral-800 bg-neutral-900 p-3 text-center">
					...
					<div className="flex items-center gap-4">
						<label className="flex items-center gap-2 text-sm text-neutral-400 cursor-pointer">
							<input
								...
								checked={reqHalfDay}
								onChange={(e) =>
									setReqHalfDay(e.target.checked)
								}
