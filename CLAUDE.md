...
- `netlify/functions/` — service-role-only ops: `create-business`, `admin-invite-user` (master in own business **or** super admin into any business via `businessId`), `admin-delete-user` (removes the auth account too), `admin-lock-user` (master in own business **or** super admin), `admin-update-password`, `admin-update-email` (changes the auth login email **and** the `profiles.email` copy), `login-rate-limit`, `send-push`
- `supabase/` — `1_schema.sql`, `2_seed.sql`, then `3_migration.sql` → `31_migration.sql` (run in order)
